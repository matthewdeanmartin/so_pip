...
if os.path.exists(CONFIG_PATH):
    ...
    print(f"Did not fine a .so_pip.ini file anywhere, using default behaviors.")
...
KEEP_ANSWERS_WITH_NO_CODE = ast.literal_eval(section.get("KEEP_ANSWERS_WITH_NO_CODE","True"))
...
COMMENT_OUT_BAD_PYTHON = ast.literal_eval(section.get("COMMENT_OUT_BAD_PYTHON","False"))
# one liners are often english with <code/> used for styling
ASSUME_ONE_LINER_IS_NOT_CODE = ast.literal_eval(section.get("ASSUME_ONE_LINER_IS_NOT_CODE","True"))
...
GENERATE_REQUIREMENTS_TXT = ast.literal_eval(section.get("GENERATE_REQUIREMENTS_TXT", "True"))
...
GENERATE_CODE_OF_CONDUCT = ast.literal_eval(section.get("GENERATE_CODE_OF_CONDUCT", "True"))
...
DEFAULT_LANGUAGE = cast(Tuple[str, str], ast.literal_eval(section.get("DEFAULT_LANGUAGE","'.py'")))
